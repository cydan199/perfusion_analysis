...
hold(ax, 'on')
histogram(ax, flattened_data, 'Normalization', 'pdf','NumBins',150); % , 'DisplayStyle', 'stairs', 'EdgeColor', 'k'
plot(ax, x_vals, pdf1, 'r', 'LineWidth', 2);
plot(ax,x_vals, pdf2, 'b', 'LineWidth', 2);
plot(ax,intersection_points, values_f1, 'go', 'MarkerSize', 10, 'LineWidth', 2);
legend(ax,'Data Histogram', 'Gaussian 1', 'Gaussian 2', 'Intersection Points');
xlim(ax, [0,1])
title(ax,'GMM Fit and Intersection Points');
xlabel(ax,'Data Values');
ylabel(ax,'Density');
hold(ax, 'off')
